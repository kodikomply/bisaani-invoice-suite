...
On first login, the initial administrator is created from `INITIAL_ADMIN_NAME`, `INITIAL_ADMIN_EMAIL`, and `INITIAL_ADMIN_PASSWORD`. Remove the bootstrap password from the server environment after that account exists. Administrators can then use **User Management** to create staff, manager, viewer, or additional administrator accounts.
